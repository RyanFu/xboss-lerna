...
exports.ASSET_REGEXP = /\.(png|jpe?g|gif|webp|ico|jfif|svg|woff2?|ttf)$/i;
...
function getComponents() {
    ...
    return dirs
        ...
        const path = (0, path_1.join)(constant_1.SRC_DIR, dir, `index.${ext}`);
...
function isAsset(path) {
    return exports.ASSET_REGEXP.test(path);
}
...
function getWebpackConfig(defaultConfig) {
    if ((0, fs_extra_1.existsSync)(constant_1.ROOT_WEBPACK_CONFIG_FILE)) {
        ...
        if (typeof config === 'function') {
            return (0, webpack_merge_1.merge)(defaultConfig, config(defaultConfig));
        }
        return (0, webpack_merge_1.merge)(defaultConfig, config);
    }
