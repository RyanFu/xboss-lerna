...
function getDeps(component) {
    ...
    if (styleDepsJson.map[component]) {
        ...
        if ((0, gen_style_deps_map_1.checkStyleExists)(component)) {
            deps.push(component);
        }
        return deps;
    }
...
function getPath(component, ext = '.css') {
    return (0, path_1.join)(constant_1.ES_DIR, `${component}/index${ext}`);
}
function getRelativePath(component, style, ext) {
    return (0, path_1.relative)((0, path_1.join)(constant_1.ES_DIR, `${component}/style`), getPath(style, ext));
}
const OUTPUT_CONFIG = [
    {
...
function genEntry(params) {
    ...
    OUTPUT_CONFIG.forEach(({ dir, template }) => {
        const outputDir = (0, path_1.join)(dir, component, 'style');
        const outputFile = (0, path_1.join)(outputDir, filename);
        let content = '';
        if (baseFile) {
            const compiledBaseFile = (0, common_1.replaceExt)(baseFile.replace(constant_1.SRC_DIR, dir), ext);
            content += template((0, path_1.relative)(outputDir, compiledBaseFile));
            content += '\n';
        }
        content += depsPath.map(template).join('\n');
        content = content.replace(new RegExp('\\' + path_1.sep, 'g'), '/');
        (0, fs_extra_1.outputFileSync)(outputFile, content);
    });
}
function genComponentStyle(options = { cache: true }) {
    if (!options.cache) {
        delete require.cache[constant_1.STYLE_DEPS_JSON_FILE];
    }
    const components = (0, common_1.getComponents)();
    const baseFile = (0, css_1.getCssBaseFile)();
