...
function exists(filePath) {
    if (!(filePath in existsCache)) {
        existsCache[filePath] = (0, fs_extra_1.existsSync)(filePath);
    }
...
function getPathByImport(code, filePath) {
    ...
    if (relativePath.includes('.')) {
        return fillExt((0, path_1.join)(filePath, '..', relativePath));
    }
    return null;
}
...
function getDeps(filePath) {
    ...
    const code = (0, fs_extra_1.readFileSync)(filePath, 'utf-8');
